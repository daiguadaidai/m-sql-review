...
func (s *testColumnChangeSuite) TestFailBeforeDecodeArgs(c *C) {
	...
	tc.onJobRunBefore = func(job *model.Job) {
		...
		if job.SchemaState == model.StateWriteOnly {
			...
			if first {
				gofail.Enable("github.com/daiguadaidai/m-sql-review/dependency/ddl/errorBeforeDecodeArgs", `return(true)`)
				first = false
			} else {
				gofail.Disable("github.com/daiguadaidai/m-sql-review/dependency/ddl/errorBeforeDecodeArgs")
			}
		}
	}
	d.SetHook(tc)
