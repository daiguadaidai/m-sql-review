...
func (s *testStateChangeSuite) TestInitializeOffsetAndState(c *C) {
	...
	gofail.Enable("github.com/daiguadaidai/m-sql-review/dependency/ddl/uninitializedOffsetAndState", `return(true)`)
	_, err = s.se.Execute(context.Background(), "ALTER TABLE t MODIFY COLUMN b int FIRST;")
	c.Assert(err, IsNil)
	gofail.Disable("github.com/daiguadaidai/m-sql-review/dependency/ddl/uninitializedOffsetAndState")
}

func (s *testDBSuite) TestUpdateHandleFailed(c *C) {
	gofail.Enable("github.com/daiguadaidai/m-sql-review/dependency/ddl/errorUpdateReorgHandle", `return(true)`)
	defer gofail.Disable("github.com/daiguadaidai/m-sql-review/dependency/ddl/errorUpdateReorgHandle")
...
func (s *testDBSuite) TestAddIndexFailed(c *C) {
	gofail.Enable("github.com/daiguadaidai/m-sql-review/dependency/ddl/mockAddIndexErr", `return(true)`)
	defer gofail.Disable("github.com/daiguadaidai/m-sql-review/dependency/ddl/mockAddIndexErr")
	tk := testkit.NewTestKit(c, s.store)
